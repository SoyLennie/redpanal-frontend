...
import {
  ...
  ChevronUp, ChevronDown, X, GitBranch, MoreHorizontal,
} from 'lucide-react';
import { useNavigate } from 'react-router-dom';
import { useAppStore } from '@/store/appStore';
import type { PlayerState } from '@/store/appStore';
...
export function Player() {
  ...
  const dragStartState = useRef<PlayerState>('mini');

  const handleDragStart = (e: React.TouchEvent | React.MouseEvent) => {
    const target = e.target as HTMLElement;
    // Don't start drag when interacting with buttons/inputs
    if (target.closest('button') || target.closest('input')) return;
    const y = 'touches' in e ? e.touches[0].clientY : e.clientY;
    dragStartY.current = y;
    dragStartState.current = playerState;
  };

  const handleDragEnd = (e: React.TouchEvent | React.MouseEvent) => {
    if (dragStartY.current === null) return;
    const y = 'changedTouches' in e ? e.changedTouches[0].clientY : e.clientY;
    const delta = dragStartY.current - y; // positive = swipe up, negative = swipe down

    if (dragStartState.current === 'mini' && delta > 50) setPlayerState('half');
    else if (dragStartState.current === 'half' && delta > 80) setPlayerState('full');
    else if (dragStartState.current === 'half' && delta < -80) setPlayerState('mini');
    else if (dragStartState.current === 'full' && delta < -80) setPlayerState('half');
  ...
  useEffect(() => {
    ...
    if (!currentTrack?.audioUrl) { audio.pause(); return; }
  ...
  useEffect(() => {
    ...
    const onEnded = () => {
      ...
      else { useAppStore.setState({ isPlaying: false, progress: 0 }); audio.currentTime = 0; }
    };
    const onError = () => useAppStore.setState({ isPlaying: false });
  ...
  if (!currentTrack) {
    ...
    return (
      <>
        <div className="hidden md:block fixed left-56 right-0 bottom-2" style={{ zIndex: Z.playerMini }}>
          {emptyBar}
        </div>
        <div
          className="md:hidden fixed left-0 right-0"
          style={{ zIndex: Z.playerMini, bottom: 'calc(72px + env(safe-area-inset-bottom, 0px))' }}
        >
          {emptyBar}
        </div>
      </>
  ...
  // ── Mini bar inner JSX (shared between mobile + desktop) ─────────────────
  const miniBarInner = (
    <>
          ...
          <div className="flex items-end gap-px h-5">
            {[8, 14, 10, 16, 9].map((h, i) => (
              <div
                ...
                style={{ height: `${h}px`, animation: `eqbar 0.7s ease-in-out infinite alternate`, animationDelay: `${i * 0.12}s` }}
              />
            ))}
          </div>
        )}
  ...
  // ── Full player content (shared mobile sheet + desktop overlay) ───────────
  const fullPlayerContent = (onClose: () => void, onBack: () => void) => (
    <div className="h-full bg-gradient-to-t from-navy-900 via-navy-900 to-[#1a2a3d] overflow-y-auto">
      {/* Drag handle */}
      <div
        className="flex justify-center pt-3 pb-2 sticky top-0 bg-navy-900/90 backdrop-blur-sm z-10 cursor-grab active:cursor-grabbing"
      ...
      <div className="px-6 flex items-center justify-between">
        <button onClick={onBack}><ChevronDown className="w-6 h-6 text-gray-400" /></button>
      ...
      <div className="px-8 pt-5 text-center">
        ...
        <button
          onClick={() => { onClose(); navigate(`/${currentTrack.artist.replace(/^@/, '')}`); }}
  ...
  return (
    <>
      {/* ─── Desktop mini bar (md+) ─────────────────────────────────────────── */}
      <div
        className="hidden md:block fixed left-56 right-0 bottom-0"
        style={{ zIndex: Z.playerMini }}
      >
        <div
          onClick={() => setPlayerState('full')}
          className="mx-4 mb-2 glass-strong rounded-2xl overflow-hidden cursor-pointer hover:border-cyan-500/30 transition-colors"
        >
          {miniBarInner}
        </div>
      </div>
      ...
      {/* ─── Mobile gesture sheet (half + full) ─────────────────────────────── */}
      <div
        className="md:hidden fixed left-0 right-0 bottom-0 overflow-hidden"
        style={{
          zIndex: Z.playerExpanded,
          height: playerState === 'mini' ? 0 : playerState === 'half' ? '55vh' : '100dvh',
          borderRadius: playerState === 'full' ? '0' : '24px 24px 0 0',
          transition: 'height 0.35s cubic-bezier(0.32, 0.72, 0, 1), border-radius 0.35s cubic-bezier(0.32, 0.72, 0, 1)',
        }}
      >
        {/* ── Half state ─── */}
        {playerState === 'half' && (
          <div className="h-full bg-gradient-to-t from-navy-900 to-[#1a2a3d] flex flex-col">
            {/* Drag handle */}
            <div
            ...
            <div className="px-5 flex items-center justify-between flex-shrink-0">
              ...
              <div className="text-center">
                <p className="text-xs text-tertiary">Reproduciendo</p>
                <p className="text-xs text-cyan-400 font-medium truncate max-w-[180px]">
                  {currentTrack.instrument} · {currentTrack.genre}
                </p>
              </div>
              <button
                onClick={() => setPlayerState('full')}
            ...
            {/* Artwork + info */}
            <div className="flex items-center gap-4 px-5 pt-4 flex-shrink-0">
              <div className={`w-16 h-16 rounded-2xl bg-gradient-to-br ${typeColors[currentTrack.type]} flex items-center justify-center flex-shrink-0`}>
                <span className="text-white font-bold text-xs">{typeLabels[currentTrack.type]}</span>
              </div>
              <div className="flex-1 min-w-0">
                <h3 className="text-white font-bold text-base truncate">{currentTrack.title}</h3>
                <button
                  onClick={() => { setPlayerState('mini'); navigate(`/${currentTrack.artist.replace(/^@/, '')}`); }}
                  className="text-cyan-400 text-sm hover:underline"
                >{currentTrack.artist}</button>
              </div>
              <button
                onClick={() => setLiked(l => !l)}
                className={`w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0 ${liked ? 'bg-rose-500/20' : 'bg-white/5'}`}
              >
                <Heart className={`w-4 h-4 ${liked ? 'text-rose-400 fill-rose-400' : 'text-gray-400'}`} />
              </button>
            </div>
            ...
            <div className="px-5 pt-5 flex-shrink-0">
              ...
              <div className="flex justify-between mt-1.5 text-xs text-tertiary">
                <span>{formatTime(currentSeconds)}</span>
                <span>{formatTime(totalSeconds)}</span>
              </div>
            </div>
            ...
            {/* Controls */}
            <div className="flex items-center justify-center gap-6 pt-5 flex-shrink-0">
              <button
                onClick={prevTrack}
        ...
        {/* ── Full state (mobile) ─── */}
        {playerState === 'full' && fullPlayerContent(
          () => setPlayerState('mini'),
          () => setPlayerState('half'),
        )}
      </div>

      {/* ─── Desktop full player overlay (hidden on mobile) ─────────────────── */}
      {playerState === 'full' && (
        <div className="hidden md:flex fixed inset-0 items-end" style={{ zIndex: Z.playerExpanded }}>
          <div className="absolute inset-0 bg-black/80 backdrop-blur-sm" onClick={() => setPlayerState('mini')} />
          <div className="relative w-full rounded-t-3xl overflow-hidden max-h-[92vh]">
            {fullPlayerContent(
              () => setPlayerState('mini'),
              () => setPlayerState('mini'),
            )}
